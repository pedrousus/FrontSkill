...
import {respAdaptacion} from '../assets/PreguntasCiencias';
import { scoreRequest } from '../api/task'



  
export default function LecAdap () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respAdaptacion[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion2Adaptacion");
      setRespuestaServidor(response.respAdaptacion);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respAdaptacion.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respAdaptacion[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 2: Adaptacion de los Animales</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respAdaptacion.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respAdaptacion.length}</h2>
