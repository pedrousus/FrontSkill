...
import { respCine } from '../assets/PreguntasCiencias'
    
  
export default function LecCine () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respCine[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion1Cinematica");
      setRespuestaServidor(response.respCine);
    } catch (error) {
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 1: Cinemática</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respCine.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respCine.length}</h2>
