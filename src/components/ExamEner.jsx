...
import { respEner } from '../assets/PreguntasCiencias'


  
export default function LecEner () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respEner[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      ...
      setRespuestaServidor(response.respEner);
    } catch (error) {
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 3: Energía y Trabajo</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respEner.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respEner.length}</h2>
          <a href='/Fisica'>Regresar</a>
          </>:<></>
          }
        </section>
      </main>
