...
import {respCons} from '../assets/PreguntasCiencias';
import { scoreRequest } from '../api/task'



  
export default function LecConservacion () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respCons[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion4Conservacion");
      setRespuestaServidor(response.respCons);
    } catch (error) {
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 4: Conservación de la Fauna</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respCons.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respCons.length}</h2>
