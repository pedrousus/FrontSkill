...
import {respTiposHabit} from '../assets/PreguntasCiencias';
import { scoreRequest } from '../api/task'



  
export default function LecTiposHabit () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respTiposHabit[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion1TiposHabitat");
      setRespuestaServidor(response.respTiposHabit);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respTiposHabit.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respTiposHabit[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 1: Tipos de Habitat</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respTiposHabit.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respTiposHabit.length}</h2>
