...
import { respReacciones } from '../assets/PreguntasCiencias'



  
export default function LecReacciones () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respReacciones[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      ...
      setRespuestaServidor(response.respReacciones);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respReacciones.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respReacciones[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 3: Reacciones Químicas</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respReacciones.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respReacciones.length}</h2>
