...
import { respEnlaces } from '../assets/PreguntasCiencias'



  
export default function LecEnlaces () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respEnlaces[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion2Enlaces");
      setRespuestaServidor(response.respEnlaces);
    } catch (error) {
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 2: Enlaces Químicos</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respEnlaces.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respEnlaces.length}</h2>
