...
import { respDigestivo } from '../assets/PreguntasCiencias'

  
export default function LecDigestivo () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respDigestivo[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion1Digestivo");
      setRespuestaServidor(response.respDigestivo);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respDigestivo.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respDigestivo[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 1: Sistema Digestivo</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respDigestivo.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respDigestivo.length}</h2>
