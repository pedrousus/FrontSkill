...
import {respPartes} from '../assets/PreguntasCiencias';
import { scoreRequest } from '../api/task'


  
export default function LecPartes () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respPartes[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion4Partes");
      setRespuestaServidor(response.respPartes);
    } catch (error) {
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 4: Partes de una Planta</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respPartes.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respPartes.length}</h2>
