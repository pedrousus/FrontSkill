...
import { respCirculatorio } from '../assets/PreguntasCiencias' 

  
export default function Lec2Circulatorio () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respCirculatorio[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion2Circulatorio");
      setRespuestaServidor(response.respCirculatorio);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respCirculatorio.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respCirculatorio[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 2: Sistema Circulatorio</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respCirculatorio.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respCirculatorio.length}</h2>
          <a href='/CuerpoHumano'>Regresar</a>
          </>:<></>
          }
        </section>
      </main>
