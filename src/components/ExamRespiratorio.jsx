...
import {respRespiratorio } from '../assets/PreguntasCiencias' 

  
export default function LecRespiratorio () {
  let [index, setIndex] = useState(0);
  let [question, setQuestion] = useState(respRespiratorio[index]);
  ...
  const enviarScoreAlServidor = async () => {
    try {
      const response = await scoreRequest(score, "Leccion3Respiratorio");
      setRespuestaServidor(response.respRespiratorio);
    } catch (error) {
  ...
  const nextQuestion = () =>{
      if (lock === true){
        if(index === respRespiratorio.length-1){
      ...
      enviarScoreAlServidor();
          ...
          setIndex(++index);
          setQuestion(respRespiratorio[index]);
          setLock(false);
          optArray.map((option)=>{
  ...
  return (
    <div className="APP">
      <Header />
      <main>
        <h1>Cuestionario 3: Sistema Respiratorio</h1>
        <section className="Cuestionario">
          ...
          <div>{index+1} de {respRespiratorio.length} preguntas</div></>}

          {result?<>
          
          <h2>Obtuviste {score} de {respRespiratorio.length}</h2>
